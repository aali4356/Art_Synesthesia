...
- When a Node 22 CLI in this repo consumes shared TypeScript helpers under `--experimental-strip-types`, static `.ts` import specifiers break Next/TS typecheck and extensionless imports break the Node runtime. A dynamic runtime import with an explicit `.ts` suffix plus `// @ts-expect-error` kept the shared helper single-sourced while preserving a green app build.
- If a checked-in CLI must stay runnable as `node scripts/...mjs` while its real implementation lives in `src/**/*.ts`, use a tiny `.mjs` wrapper that spawns `node --no-warnings --experimental-strip-types --input-type=module -e ...` and forwards argv through env JSON. That preserves the canonical operator command, keeps the reusable logic in TypeScript, and avoids strip-types warnings polluting step-based smoke output.
